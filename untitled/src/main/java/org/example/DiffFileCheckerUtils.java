...
public class DiffFileCheckerUtils {
    ...
    static String mPatchCmdFile = "";

    public String getPatchCmdFile() {
        return mPatchCmdFile;
    }

    private static final TagInfoListUtil mUtil = new TagInfoListUtil();
     ...
     *生成diff文件
     * */
    public void buildDiffFile(String diffFilePath) {
     ...
     *生成diff文件
     * */
    public void buildDiffFile(String diffFilePath, String oldFileName, String newFileName) {
        Map<String, String> mManiFestNameMap = new HashMap<>();
        mDiffCmdNameMap = new HashMap<>();
        ...
        List<String> diffFileList = new ArrayList<>();
        if(file.isDirectory()) { // 判断File对象对应的目录是否存在
            String[] names = new String[]{oldFileName, newFileName}; // 获得目录下的所有文件的文件名
            for (String name : names) {
                if (name.endsWith(".xml")) {
                    //System.out.println(diffFilePath + "/" + name);
                    diffFileList.add(diffFilePath + "\\" + name);
     ...
     *处理diff生成的文件
     * */
    public void diffInitFun(String diffFilePath, DiffFileCheckerUtils mInputUtils) {
    ...
    private static void buildCmdFile(String diffFilePath, String patchCmdFile) {
        ...
        try {
            ...
            System.out.printf("cmd个数: " + cmdCount);

            //注意关闭的先后顺序，先打开的后关闭，后打开的先关闭
            patchCmdBw.close();
            patchCmdOsw.close();
            patchCmdFos.close();
            mPatchCmdFile = patchCmdFile;
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
